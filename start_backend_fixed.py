import paramiko
import time

# Config
REMOTE_HOST = "64.90.1.179"
REMOTE_USER = "root"
REMOTE_PASS = "sqdSndJ2r2nqaGfu"
REMOTE_DIR = "/root/AgentProject"

print("Connecting to server...")
ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect(REMOTE_HOST, username=REMOTE_USER, password=REMOTE_PASS, timeout=15, allow_agent=False, look_for_keys=False)

print("[OK] Connected\n")

# Step 1: Kill existing processes
print("=" * 60)
print("[Step 1] Killing existing Streamlit processes...")
ssh.exec_command("pkill -f streamlit || true", timeout=5)
print("[OK] Done")
print("=" * 60)

# Step 2: Check if app/main.py exists
print("\n[Step 2] Verifying file exists...")
stdin, stdout, stderr = ssh.exec_command(f"ls -lh {REMOTE_DIR}/app/main.py", timeout=5)
result = stdout.read().decode().strip()
if result:
    print(f"[OK] {result}")
else:
    print(f"[ERROR] File not found: {REMOTE_DIR}/app/main.py")
    ssh.close()
    exit(1)
print("=" * 60)

# Step 3: Create a startup script
print("\n[Step 3] Creating startup script...")
startup_script = f"""#!/bin/bash
cd {REMOTE_DIR}
streamlit run {REMOTE_DIR}/app/main.py --server.port 8501 --server.address 0.0.0.0 > /tmp/streamlit.log 2>&1
"""

stdin, stdout, stderr = ssh.exec_command(f"cat > {REMOTE_DIR}/start.sh << 'EOF'\n{startup_script}\nEOF\nchmod +x {REMOTE_DIR}/start.sh", get_pty=True, timeout=10)
print("[OK] Startup script created")

# Step 4: Start backend using the script
print("\n[Step 4] Starting backend...")
cmd = f"cd {REMOTE_DIR} && nohup bash {REMOTE_DIR}/start.sh &"
stdin, stdout, stderr = ssh.exec_command(cmd, timeout=10)
print(f"[OK] Backend starting...")

# Step 5: Wait
print("\n[Step 5] Waiting for backend to start (15s)...")
time.sleep(15)

# Step 6: Check process
print("\n[Step 6] Checking if backend is running...")
stdin, stdout, stderr = ssh.exec_command("ps aux | grep 'streamlit run' | grep -v grep", timeout=10)
result = stdout.read().decode().strip()
if result:
    print("[OK] Backend is running:")
    print(result)
else:
    print("[ERROR] Backend NOT running")
    stdin, stdout, stderr = ssh.exec_command("tail -30 /tmp/streamlit.log", timeout=10)
    log = stdout.read().decode().strip()
    print(f"Log:\n{log}")
    ssh.close()
    exit(1)

# Step 7: Check port
print("\n[Step 7] Checking port 8501...")
stdin, stdout, stderr = ssh.exec_command("netstat -tlnp 2>/dev/null | grep 8501 || ss -tlnp 2>/dev/null | grep 8501", timeout=10)
result = stdout.read().decode().strip()
if result:
    print(f"[OK] Port 8501 is open:\n{result}")
else:
    print("[WARN] Port 8501 not found in netstat")

# Step 8: Test HTTP
print("\n[Step 8] Testing HTTP endpoint...")
stdin, stdout, stderr = ssh.exec_command("curl -s -o /dev/null -w '%{http_code}' http://localhost:8501", timeout=10)
result = stdout.read().decode().strip()
if result == "200":
    print("[OK] Backend HTTP endpoint is accessible (200)")
else:
    print(f"[WARN] HTTP status: {result}")
    print("Checking log again...")
    stdin, stdout, stderr = ssh.exec_command("tail -10 /tmp/streamlit.log", timeout=10)
    print(stdout.read().decode().strip())

ssh.close()

print("\n" + "=" * 60)
print("BACKEND DEPLOYED SUCCESSFULLY!")
print("=" * 60)
print(f"Backend URL: http://{REMOTE_HOST}:8501")
print(f"Access the web UI at: http://{REMOTE_HOST}:8501")
print("=" * 60)
